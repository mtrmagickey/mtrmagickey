  ...
  return {
    ...
    transformIndexHtml: {
      ...
      async transform(html, ctx) {
        ...
        const template = handlebars$1.compile(html, compileOptions); // Lets load in the glitch settings.json

        const settings = await promises.readFile(settingsFile, 'utf-8');
        let json;
        ...
        try {
          ...
          json = async () => Promise.resolve().then(function () { return require('./settings-12726a44.js'); });
        }

        const resolvedContext = await resolveContext({
          settings: json
        }, vite.normalizePath(ctx.path));
